...
function cyberchimps_core_scripts() {
	...
	$cyberchimps_protocol = is_ssl() ? 'https' : 'http';
	wp_register_style( 'cyberchimps-font-styles', $cyberchimps_protocol . '://fonts.googleapis.com/css?family=Noto+Sans:400,700' );
	wp_enqueue_style( 'cyberchimps-font-styles' );
...
function cyberchimps_recent_post_excerpt_more( $more ) {
	...
	return '&hellip;
			...
			<div class="more-link">
				<a href="' . esc_url(get_permalink( $post->ID )) . '">' . cyberchimps_blog_read_more_text() . '</a>
			</div>';
}
...
function cyberchimps_featured_post_excerpt_more( $more ) {
	...
	return '&hellip;</p></span><a class="excerpt-more featured-post-excerpt" href="' . esc_url(get_permalink( $post->ID )) . '">' . cyberchimps_blog_read_more_text() . '</a>';
}
...
function cyberchimps_magazine_featured_post_excerpt_more( $more ) {
	...
	return '&hellip;</p></span><a class="excerpt-more magazine-featured-post-excerpt" href="' . esc_url(get_permalink( $post->ID )) . '">' . cyberchimps_blog_read_more_text() . '</a>';
}
...
function cyberchimps_search_excerpt_more( $more ) {
	...
	if( get_theme_mod( 'search_post_read_more' ) != '' ) {
		$more = '<p><a href="' . esc_url(get_permalink( $post->ID )) . '">' . get_theme_mod( 'search_post_read_more' ) . '</a></p>';

		return $more;
	}
	else {
		$more = '<p><a class="excerpt-more search-excerpt" href="' . esc_url(get_permalink( $post->ID )) . '">' . __( 'Read More...', 'cyberchimps_core' ) . '</a></p>';
...
function cyberchimps_archive_excerpt_more( $more ) {
	global $post;
	return '<p><a class="excerpt-more archive-excerpt" href="' . esc_url(get_permalink( $post->ID )) . '">' . cyberchimps_blog_read_more_text() . '</a></p>';
}

add_action( 'pre_get_posts', 'cyberchimps_check_archive_page' );
function cyberchimps_check_archive_page($query)
{
	if( ! is_admin() && $query->is_main_query() && $query->is_archive == '1' )
	{
		if( get_theme_mod( 'archive_post_excerpts', 0 ) != 0 ) {
		add_filter( 'excerpt_more', 'cyberchimps_archive_excerpt_more', 999 );
		}
	}
}
...
function cyberchimps_blog_excerpt_more( $more ) {
	global $post;
	return '<p><a class="excerpt-more blog-excerpt" href="' . esc_url(get_permalink( $post->ID )) . '">' . cyberchimps_blog_read_more_text() . '</a></p>';
...
function manual_excerpt_read_more_link( $output ) {
	...
	if( !empty( $post->post_excerpt ) ) {
		return $output . '<p><a class="excerpt-more" href="' . esc_url(get_permalink( $post->ID )) . '">' . $linktext . '</a></p>';
	}
	else {
		return $output;
	}
