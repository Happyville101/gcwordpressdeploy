...
function cyberchimps_header_logo() {
	...
	if ( has_custom_logo( $blog_id = 0 )) {
		...
		?>
		<div id="logo">
			<a href="<?php echo esc_url($url); ?>" title="<?php echo get_bloginfo( 'name' ); ?>"><img src="<?php echo $logo[0] ? $logo[0] : apply_filters('cyberchimps_default_logo' , $url); ?>" alt="<?php echo get_bloginfo( 'name' ); ?>"></a>
		</div>
	<?php
	} else {
...
function cyberchimps_banner_content() {
	...
	<header id="cc-header" class="row">
		<div id="banner">
			<?php if ( $banner != "" ): ?>
				<a href="<?php echo esc_url($url); ?>"><img src="<?php echo esc_url($banner); ?>" alt="logo"></a>
			<?php endif; ?>
			<?php if ( $banner == "" ): ?>
